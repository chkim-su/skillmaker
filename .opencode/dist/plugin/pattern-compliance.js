...
function checkContentPatterns(content, fileType) {
    const issues = [];
    ...
    const stageCount = stagePatterns.reduce((sum, p) => sum + (content.match(p)?.length || 0), 0);
    if (stageCount >= 3) {
...
function printAlert(context, name, issues) {
    if (issues.length === 0)
        return;
    const lines = [
        `Context: ${context}`,
...
export const PatternCompliance = async ({ directory }) => {
