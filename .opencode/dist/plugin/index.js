...
function findPluginRoot(startPath) {
...
function printBox(title, lines, icon = '') {
    console.log('');
...
const SkillmakerPlugin = async ({ directory }) => {
    ...
    const hooks = {
        'chat.params': async (input, output) => {
            if (!rules)
                return;
            const prompt = input.message?.parts?.[0]?.text || '';
            if (!prompt)
                return;
