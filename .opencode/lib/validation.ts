...
import { findPluginRoot, readJsonFile, readTextFile, printBox } from './utils.js'
...
const SKILL_REFERENCES: Record<string, SkillReference> = {
...
function getSkillHint(code: string): string {
...
function validateMarketplaceJson(pluginRoot: string): ValidationResult {
  const result: ValidationResult = { errors: [], warnings: [], passed: [] }
  ...
  const data = readJsonFile<MarketplaceJson>(marketplacePath)
  if (!data) {
    result.errors.push('[E002] marketplace.json is not valid JSON')
    return result
  }

  if (!data.name) result.errors.push('[E003] marketplace.json missing "name" field')
  if (!data.owner?.name) result.warnings.push('[W001] marketplace.json missing "owner.name"')
  ...
  for (const plugin of data.plugins) {
    if (!plugin.name) result.errors.push('[E005] Plugin missing "name"')
...
function validateSkills(pluginRoot: string): ValidationResult {
  const result: ValidationResult = { errors: [], warnings: [], passed: [] }
  ...
  for (const skillName of readdirSync(skillsDir)) {
    ...
    if (!frontmatter) {
      ...
      if (!frontmatter.name) result.warnings.push(`[W029] Skill "${skillName}" frontmatter missing "name"${getSkillHint('W029')}`)
      if (!frontmatter.description) result.warnings.push(`[W029] Skill "${skillName}" frontmatter missing "description"${getSkillHint('W029')}`)
    }
...
function validateAgents(pluginRoot: string): ValidationResult {
  const result: ValidationResult = { errors: [], warnings: [], passed: [] }
  ...
  for (const agentFile of readdirSync(agentsDir)) {
    ...
    if (!frontmatter) {
      ...
      if (!frontmatter.name) result.warnings.push(`[W030] Agent "${agentName}" frontmatter missing "name"`)
      if (!frontmatter.description) result.warnings.push(`[W030] Agent "${agentName}" frontmatter missing "description"`)
...
function analyzeKeywordContext(content: string, keyword: string): KeywordMatch[] {
...
function validateHookify(pluginRoot: string): ValidationResult {
  const result: ValidationResult = { errors: [], warnings: [], passed: [] }
  ...
  const filesWithEnforcement: Array<{ file: string, matches: KeywordMatch[] }> = []
  ...
  for (const dir of dirsToCheck) {
    ...
    for (const file of readdirSync(dirPath)) {
      ...
      const fileMatches: KeywordMatch[] = []
...
export function validateAll(pluginRoot?: string): ValidationResult {
